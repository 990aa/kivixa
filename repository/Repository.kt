...
import com.kivixa.domain.Page
import com.kivixa.domain.StrokeChunk
import java.util.concurrent.ConcurrentHashMap

data class PageKey(val documentId: Long, val pageNumber: Int)
...
class Repository(
    ...
    private val pageCache = ConcurrentHashMap<PageKey, Page>()

    suspend fun getPageByNumber(documentId: Long, pageNumber: Int): Page? {
        val key = PageKey(documentId, pageNumber)
        return pageCache[key] ?: run {
            val page = pageDao.getPageByNumber(documentId, pageNumber)?.toDomain()
            page?.let { pageCache[key] = it }
            page
        }
    }

    suspend fun invalidatePage(documentId: Long, pageNumber: Int) {
        pageCache.remove(PageKey(documentId, pageNumber))
    }
