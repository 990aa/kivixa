...
fn extract_lib_names(out_dir: &Path, build_shared_libs: bool, target_os: &TargetOs) -> Vec<String> {
    // Use target_os instead of cfg! macros to support cross-compilation
    // When cross-compiling from Windows to Android, cfg!(windows) is true but we need *.a files
...
fn main() {
    ...
    // if native isn't specified, enable specific features for ggml instead
        ...
        // CARGO_CFG_TARGET_FEATURE may not be set for Android targets
    ...
    if matches!(target_os, TargetOs::Android) {
        ...
        // Extract API level from ANDROID_PLATFORM (e.g., "android-23" -> "23")
        let api_level = android_platform.strip_prefix("android-").unwrap_or("23");
        
        // Determine the correct target triple for each ABI with API level
        // This is critical: cmake-rs adds --target without API level, which breaks linking
        // We must explicitly set CMAKE_C/CXX_FLAGS to override this
        let android_target = match android_abi {
            "arm64-v8a" => format!("aarch64-linux-android{}", api_level),
            "armeabi-v7a" => format!("armv7a-linux-androideabi{}", api_level),
            "x86_64" => format!("x86_64-linux-android{}", api_level),
            "x86" => format!("i686-linux-android{}", api_level),
            _ => format!("aarch64-linux-android{}", api_level),
        };
        
        // Override CMAKE_CXX_FLAGS to include correct --target with API level
        // This prevents the linker from failing to find sysroot files (crtbegin_dynamic.o, libc++, etc.)
        config.define("CMAKE_C_FLAGS", &format!("--target={}", android_target));
        config.define("CMAKE_CXX_FLAGS", &format!("--target={}", android_target));
        config.define("CMAKE_ASM_FLAGS", &format!("--target={}", android_target));
