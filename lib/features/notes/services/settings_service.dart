...
class SettingsService {
  ...
  static const String _palmRejectionSensitivityKey =
      '${_prefix}palmRejectionSensitivity';
  ...
  Future<void> saveSettings(NotesSettings settings) async {
    ...
    await prefs.setInt(
      _defaultPenColorKey,
      settings.defaultPenColor.toARGB32(),
    );
    ...
    await prefs.setDouble(
      _palmRejectionSensitivityKey,
      settings.palmRejectionSensitivity,
    );
    await prefs.setDouble(
      _pressureSensitivityKey,
    ...
    await prefs.setString(
      _documentNamePatternKey,
      settings.documentNamePattern,
    );
  }
  ...
  Future<NotesSettings> loadSettings() async {
    ...
    return NotesSettings(
      defaultPaperType: PaperType
          .values[prefs.getInt(_defaultPaperTypeKey) ?? PaperType.plain.index],
      ...
      autoSaveFrequency:
          AutoSaveFrequency.values[prefs.getInt(_autoSaveFrequencyKey) ??
              AutoSaveFrequency.sec15.index],
      paperSize:
          PaperSize.values[prefs.getInt(_paperSizeKey) ?? PaperSize.a4.index],
      exportQuality:
          ExportQuality.values[prefs.getInt(_exportQualityKey) ??
              ExportQuality.medium.index],
      stylusOnlyMode: prefs.getBool(_stylusOnlyModeKey) ?? false,
      palmRejectionSensitivity:
          prefs.getDouble(_palmRejectionSensitivityKey) ?? 0.5,
      ...
      autoCleanup: AutoCleanup
          .values[prefs.getInt(_autoCleanupKey) ?? AutoCleanup.never.index],
      maxStorageLimit: prefs.getDouble(_maxStorageLimitKey) ?? 500.0,
      exportLocation: prefs.getString(_exportLocationKey) ?? '',
      documentNamePattern:
          prefs.getString(_documentNamePatternKey) ?? 'Note_{YYYY}-{MM}-{DD}',
    );
  }
}
