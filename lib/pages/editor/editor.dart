...
import 'package:kivixa/components/canvas/canvas_image.dart';
...
import 'package:kivixa/data/tools/eraser.dart';
...
import 'package:kivixa/data/tools/pencil.dart';
...
class EditorState extends State<Editor> {
  ...
  void undo([EditorHistoryItem? item]) {
    if (item == null) {
      ...
      // if I disabled redo, re-enable it
      if (!history.canRedo) {
    ...
    setState(() {
      switch (item!.type) {
        ...
        case EditorHistoryItemType.deletePage:
          // make sure I already have a (blank/otherwise) page at this index
  ...
  void onDrawUpdate(ScaleUpdateDetails details) {
    ...
    // PencilSound.update(offset.distance); // Audio removed
  ...
  void onDrawEnd(ScaleEndDetails details) {
    ...
    // PencilSound.pause(); // Audio removed
    setState(() {
      if (currentTool is Pen) {
  ...
  void _refreshCurrentNote() async {
    // Cloud storage removed - kivixa uses local storage only
    if (coreInfo.readOnly) return;
    // Simply reload from local file
    _initStrokes();
  }
  ...
  /// Exports the current note as an kvx (kivixa Archive) file.
  Future exportAsSba(BuildContext context) async {
    final kvx = await coreInfo.saveToSba(currentPageIndex: currentPageIndex);
    if (!context.mounted) return;
    await FileManager.exportFile(
      '${coreInfo.fileName}.kvx ',
      kvx,
      context: context,
    );
  }
