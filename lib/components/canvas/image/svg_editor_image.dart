...
class SvgEditorImage extends EditorImage {
  ...
  SvgEditorImage({
    ...
    if (svgString != null) {
      ...
      // On web, use asset loader with the file path
      svgLoader = SvgAssetLoader(svgFile!.path);
    } else {
      svgLoader = SvgFileLoader(svgFile! as dynamic);
    }
  }
    ...
    if (assetIndex != null) {
      if (inlineAssets == null) {
        svgFile = FileManager.getFile(
          '$kvxPath${Editor.extension}.$assetIndex',
        );
  ...
  ({String? string, Object? file}) _extractSvg() => switch (svgLoader) {
    (final SvgStringLoader loader) => (
      string: loader.provideSvg(null),
      file: null,
    ),
    (final SvgFileLoader loader) => (
      string: null,
      file: loader.file as Object?,
    ),
    (final SvgAssetLoader loader) => (string: loader.assetName, file: null),
    (_) => throw ArgumentError.value(
      svgLoader,
      'svgLoader',
      'SvgEditorImage.toJson: svgLoader must be a SvgStringLoader, SvgFileLoader, or SvgAssetLoader',
    ),
  };
  ...
  SvgEditorImage copy() {
    ...
    return SvgEditorImage(
      ...
      svgFile: svgData.file as io.File?,
      pageIndex: pageIndex,
      pageSize: Size.infinite,
