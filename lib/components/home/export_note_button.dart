...
import 'package:kivixa/components/theming/adaptive_circular_progress_indicator.dart';
  ...
  Future exportFile(List<String> selectedFiles, bool exportPdf) async {
    ...
    for (final filePath in selectedFiles) {
      ...
      if (exportPdf) {
        ...
        final kvx = await coreInfo.saveToSba(currentPageIndex: null);
        files.add(
          ArchiveFile('$fileNameWithoutExtension.kvx ', kvx.length, kvx),
        );
      }
    }
