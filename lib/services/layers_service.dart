...
class LayersService {
  ...
  Future<void> toggleLayerVisibility(int layerId) async {
    ...
    if (layer != null) {
      await _repository.updateLayer(layerId, {'visible': !(layer['visible'] as bool)});
    }
  }

  /// Reassigns selected items to a different layer.
  Future<void> reassignItemsToLayer(List<int> itemIds, int layerId, String itemType) async {
    await _repository.batchWrite([
  ...
  Future<void> _updateItemLayer(int itemId, int layerId, String itemType) {
    switch (itemType) {
      case 'stroke':
        // Assuming stroke chunks have a layer_id
        // This is a simplification. In reality, you might need to update all chunks of a stroke.
        return _repository.db.update('stroke_chunks', {'layer_id': layerId}, where: 'stroke_id = ?', whereArgs: [itemId]);
      case 'text_block':
  ...
  Future<List<int>> getZOrderList(int pageId) async {
    final layers = await _repository.listLayers(pageId: pageId, orderBy: 'z_order ASC');
