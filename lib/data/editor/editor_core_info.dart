...
class EditorCoreInfo {
  ...
  /// - 2: Store width and height in kvx
  /// - 1: Store version in kvx
  static const kvxVersion = 19;
  var readOnly = false;
  ...
  factory EditorCoreInfo.fromJson(
    ...
    final readOnlyBecauseOfVersion = fileVersion > kvxVersion;
    ...
    try {
      ...
      if (alwaysUseIsolate || length > 2 * 1024 * 1024) {
        ...
        coreInfo = await workerManager.execute(
          () async {
            // I need to rerun some "init" methods in the isolate,
            // see https://github.com/kivixa/issues/1031.
  ...
  /// Converts the current note as an kvx (kivixa Archive) file,
  /// which contains the main bson file and all the assets
  /// compressed into a zip file.
  ///
  /// In the archive, the main bson file is named `main.kvx`,
  /// and the assets are named `main.kvx.0`, `main.kvx.1`, etc.
  ///
