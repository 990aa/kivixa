...
class FolderCard(QFrame):
    ...
    def __init__(self, folder_model, parent=None):
        ...
        self.name_label = QLabel(folder_model.name)
    ...
    def _on_delete_action(self):
        self.delete_requested.emit(str(self.folder_model.id))

    def _on_rename_action(self):
        new_name, ok = QInputDialog.getText(self, "Rename Folder", "Enter new name:", text=self.folder_model.name)
        if ok and new_name:
            self.rename_requested.emit(str(self.folder_model.id), new_name)

    def _on_duplicate_action(self):
        self.duplicate_requested.emit(str(self.folder_model.id))

    def _on_move_action(self):
        self.move_requested.emit(str(self.folder_model.id))
